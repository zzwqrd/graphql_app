...
class DioServices {
  ...
  void init() {
    ...
    _dio.options = BaseOptions(
      ...
      //https://ar-qa.ajmal.com
      headers: {
        "Content-Type": "application/json",
        "Accept": "application/json",

        // مهم جدًا في Magento
        // "Store": "qatar_ar", // غيرها حسب البيئة
        // "Store": "qatar_$lang",
        ...
        // Authorization لو موجود
        if (token != null && token.isNotEmpty) "Authorization": "Bearer $token",
        // "Content-Type": "application/json",
        // "Accept": "application/json, text/plain, */*",

        // "Origin": "https://ar-qa.ajmal.com/",
        // "Referer": "https://ar-qa.ajmal.com/",
        // "User-Agent":
    ...
    if (!_dio.options.baseUrl.startsWith('http') ||
        !_dio.options.baseUrl.startsWith('https')) {
      throw Exception("Invalid baseUrl: must start with 'http '");
    }

    // _dio.options.headers.removeWhere((key, value) => value == null);
    // _dio.options.headers.removeWhere((key, value) => value.toString().isEmpty);
    // _dio.options.headers.removeWhere((key, value) => value == "null");
    // هقوله هنا لو التوكن بنل شيل         'Authorization': 'Bearer ${preferences.getString('auth_token')}',
    // _dio.options.headers.removeWhere((key, value) => value == "Bearer null");
  ...
  void updateAuthToken(String? token) {
    if (token != null) {
      ...
      _logger.green("Token updated in Dio headers");
    } else {
      _dio.options.headers.remove('Authorization');
      _logger.yellow("Token removed from Dio headers");
    }
  }
  ...
  Future<HelperResponse<T>> get<T>(
    ...
    try {
      ...
      if (requireAuth && preferences.getString('auth_token') != null) {
  ...
  Future<HelperResponse<T>> post<T>(
    ...
    try {
      ...
      if (requireAuth && preferences.getString('auth_token') != null) {
        ...
        mergedHeaders.remove("authorization");
        mergedHeaders.remove("Authorization");
      }
      ...
      _logger.blue("POST Request Data: $requestData");
      ...
      if (response.statusCode == 200 || response.statusCode == 201) {
        ...
        if ((data.toString().contains("DOCTYPE") ||
          ...
          FlashHelper.showToast(
            data is Map ? data['message'] : 'Malformed response',
          );
        ...
        // Handle different response formats
          ...
          (response.data is Map && response.data["exception"] != null))) {
        _logger.red("Malformed or unexpected response received.");
        return HelperResponse<T>.unknownError(
          statusCode: response.statusCode ?? 500,
          message: 'Malformed or unexpected response',
